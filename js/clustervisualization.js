...
function drawSummaryData(kmeandata, div_to_update, includeRainInNegative) {
...
function call_concentration_with_cutoff(file_amplitudes, thecutoff, div_to_use, div_result, filenames, includeRainInNegative) {
	var running_res = [];
	var running_total = 0.0;
	var data = new google.visualization.DataTable();
	...
	let vtemp_lower;
	if (includeRainInNegative)
		vtemp_lower = parseFloat(thecutoff);
	else
...
function display_new_form(cutoff) {
	document.getElementById("count_using_cutoff").innerHTML = `
		<h2>Analyze droplet files using the cutoff defined by the positive control above.</h1>
...
function handleFileSelectCount(evt) {
	...
	for (var i = 0, f; f = files2[i]; i++) {
		...
		reader.onload = (function (e) {
			...
			var amplitudes = csv_d.map(function (value, index) {
		...
		reader.onloadend = (function (e) {
			...
			if (call_counter == (files2.length)) {
				...
				let includeRainInNegative = document.getElementById("includeRainInNegative").checked;
				call_concentration_with_cutoff(reader.file_amplitude, current_cutoff, "to_count", "to_count_result", filenames, includeRainInNegative);
			}
		});
...
function handleFileSelect(evt) {
	...
	for (var i = 0, f; f = files[i]; i++) {
		...
		reader.onload = (function (e) {
			...
			//console.debug(e);
			var csv_d = CSVToArray(contents);
			amplitudes = csv_d.map(function (value, index) {
				return value[0];
			});
			reader.file_amplitude.push(amplitudes);
		...
		reader.onloadend = (function (e) {
			...
			if (call_counter == (files.length)) {
				...
				var cutoff = drawSummaryData(groups, "summary_positive_control", includeRainInNegative);
				current_cutoff = cutoff;
				call_concentration_with_cutoff(reader.file_amplitude, cutoff, "positive_call", "positive_result", file_names, includeRainInNegative);
